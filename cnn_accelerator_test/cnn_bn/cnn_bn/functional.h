...
template<int CH, int ROW, int COL>
void conv_bn(float in[CH][ROW][COL], float out[CH][ROW][COL], float w[CH], float b[CH]) {
    for(int ch=0; ch < CH; ch ++) {
        for(int row=0; row < ROW; row ++) {
            for(int col=0; col < COL; col ++) {
                out[ch][row][col] = in[ch][row][col] * w[ch] + b[ch];
            }
        }
    }
}
template<int LEN>
void linear_bn(float in[LEN], float out[LEN], float w[LEN], float b[LEN]) {
    for(int i=0; i < LEN; i ++) {
        out[i] = in[i] * w[i] + b[i];
    }
}
