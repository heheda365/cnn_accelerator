...
void conv_bn(int in[CH][ROW][COL], int out[CH][ROW][COL], float w[CH], float b[CH]) {
    for(int ch=0; ch < CH; ch ++) {
        for(int row=0; row < ROW; row ++) {
            for(int col=0; col < COL; col ++) {
...
template<int LEN>
void linear_bn(int in[LEN], int out[LEN], float w[LEN], float b[LEN]) {
    for(int i=0; i < LEN; i ++) {
...
void conv_bn_qrelu(int in[IN_CH][IN_ROW][IN_COL], int out[IN_CH][IN_ROW][IN_COL], float w[IN_CH], float b[IN_CH]) {
    
    for(int ch=0; ch < IN_CH; ch ++) {
        for(int row=0; row < IN_ROW; row ++) {
            for(int col=0; col < IN_COL; col ++) {
                int inc = (int)(1/w[ch] * 3 * ((1 << IN_BIT) - 1) / ((1 << OUT_BIT) - 1));
                int bias = (int)((b[ch] / w[ch]) * 3 * ((1 << IN_BIT) - 1) + inc / 2);
...
void linear_bn_qrelu(int in[LEN], int out[LEN], float w[LEN], float b[LEN]) {
    for(int i=0; i < LEN; i ++) {
        // out[i] = (int)(in[i] * w[i] + b[i] * 3 * 15);
        int inc = (int)(1/w[i] * 3 * ((1 << IN_BIT) - 1) / ((1 << OUT_BIT) - 1));
        int bias = (int)((b[i] / w[i]) * 3 * ((1 << IN_BIT) - 1) + inc / 2);
        out[i] = qrelu_search<OUT_BIT>(in[i], inc, bias);
    }
}
